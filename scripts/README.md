...
that join, and fitting models against it, plus the shared config each stage
depends on. Everything here runs in the `wetlands` mamba env, from the repo
root, as a module: `python -m scripts.<stage>.<name> ...`. `scripts` is a
real Python package now (each stage folder has an `__init__.py`), which is
why the run command is `-m scripts.model.train`, not
`python scripts/model/train.py` - the latter drops the repo root off
`sys.path` and every cross-stage import in here breaks.

`scripts/data_constants.py` sits at the package root, not in any one stage
folder: NLCD grid/class-code constants and the AlphaEarth band column list,
shared across all three stages. Plain literals, no third party imports, so
nothing pays for `rasterio`/`duckdb`/`ee` just to read a constant.

## scripts/ingest/ - pull AlphaEarth, join it onto the NLCD labels
...
whenever `TILE_SIZE_PX_30M` in `gee_common.py` changes):
`python -m scripts.ingest.build_florida_tile_grid_30m`. Writes
...
## scripts/dataset/ - build a named, versioned training pool

### build_train_pool.py

Turns the joined table into a named, versioned dataset: the spatial folds and
the weighted training pool `train.py` and `eda_dataset.ipynb` read. Every
...
`train_pool.parquet`. Each fold gets its own decay constant solved in
...
gets written to `data/processed/datasets/NAME/config.json`, including which
NLCD release and AlphaEarth dataset id it was built against, so the directory
is self documenting.
...
`model_common.py` will connect to anything. Pass `--with-neighborhood-features`
to `pool` to carry those columns into a dataset's `train_pool.parquet`
too. See `build_train_pool.py`'s `_local_density`/`_edge_mask`/
`_direction_to_nearest_developed` functions for how each feature is built and
why.

Run as `python -m scripts.dataset.build_train_pool <features|folds|pool|report> ...`.
...
Run as `python -m scripts.dataset.block_distance_profiles <profiles|cluster|report> ...`.

## scripts/model/ - fit, screen, and diagnose models
...
both `train.py` and the notebooks below loop over, the metric functions
(`best_f1_operating_point`, `calibrate`, `reliability`), and the two
diagnostics `model_analysis.ipynb` calls after loading a trained experiment
(`decile_precision_recall`, `spatial_fold_predictions`). Not runnable on its
own, plays the same role for the modeling pipeline that `gee_common.py` plays
for the AlphaEarth export. Model agnostic by design: `spatial_fold_predictions`
takes a `score_fn` per fold rather than a model object, so a future non
LightGBM experiment reuses it without a rewrite.
...
Fits and persists models for a named experiment. `baseline`
...
Only `model_type == "lightgbm"` is wired up in `make_model()`; a DL
experiment plugs in as a new branch there, see the module docstring.

Run as `python -m scripts.model.train <baseline|grid-search|grid-finalize|status> --experiment NAME ...`.
